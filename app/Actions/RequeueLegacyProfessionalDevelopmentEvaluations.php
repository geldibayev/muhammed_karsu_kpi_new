...
class RequeueLegacyProfessionalDevelopmentEvaluations
{
    ...
    public function candidates(Report $report): Builder
    {
        return Datum::query()
            ...
            ->whereNull('data.university_tier')
            ...
            ->whereDoesntHave('histories', fn (Builder $query): Builder => $query
                ->whereIn('message_type', [
                    ...
                    'criterion_2_1_5_recheck_queued',
                ]));
    }
        ...
        return DB::transaction(function () use ($datumId, $report): ?Datum {
            ...
            $datum->histories()->createMany([
                [
                    ...
                    'type' => 'info',
                    'message' => '2.1.5 kriteriyasining eski AI balli yangi Top reyting foizlari bo‘yicha qayta tekshiruvga yuborildi. '
