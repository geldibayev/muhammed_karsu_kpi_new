...
@section('content')
    ...
    <section class="content">
        <div class="container-fluid">
            ...
            @can('manage-ai-operations')
                <div class="card card-outline card-warning">
                    <div class="card-header">
                        <h2 class="card-title font-weight-bold">Eng eski baholanmagan AI resurslar</h2>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                @forelse($pendingResources as $pendingResource)
                                    <tr>
                                        <td class="align-middle">#{{ $pendingResource->id }}</td>
                                        <td class="align-middle">
                                            {{ $pendingResource->criterion?->code }} —
                                            {{ data_get($pendingResource->criterion?->name, 'uz', 'Noma’lum') }}
                                        </td>
                                        <td class="align-middle">
                                            {{ $pendingResource->user?->short ?: 'Noma’lum' }}
                                            <span class="text-muted">({{ $pendingResource->user?->hemis_id }})</span>
                                        </td>
                                        <td class="align-middle text-nowrap">{{ $pendingResource->created_at->format('d.m.Y H:i') }}</td>
                                        <td class="align-middle text-right text-nowrap">
                                            <a href="{{ route('reviews.show', $pendingResource) }}" class="btn btn-success btn-sm">
                                                <i class="fas fa-user-check mr-1"></i> Baholash
                                            </a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="5" class="text-center text-muted py-4">Baholanmagan AI resurs yo‘q.</td></tr>
                                @endforelse
                                </tbody>
                            </table>
                        </div>
                ...
                <div class="card card-outline card-info">
                    <div class="card-header">
                        <h2 class="card-title font-weight-bold">Oxirgi tasdiqlangan AI resurslar</h2>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                @forelse($acceptedResources as $acceptedResource)
                                    <tr>
                                        <td class="align-middle">#{{ $acceptedResource->id }}</td>
                                        <td class="align-middle">
                                            {{ $acceptedResource->criterion?->code }} —
                                            {{ data_get($acceptedResource->criterion?->name, 'uz', 'Noma’lum') }}
                                        </td>
                                        <td class="align-middle">
                                            {{ $acceptedResource->user?->short ?: 'Noma’lum' }}
                                            <span class="text-muted">({{ $acceptedResource->user?->hemis_id }})</span>
                                        </td>
                                        <td class="align-middle">{{ number_format((float) $acceptedResource->point, 4, '.', '') }}</td>
                                        <td class="align-middle text-right text-nowrap">
                                            <a href="{{ route('reviews.show', $acceptedResource) }}" class="btn btn-warning btn-sm">
                                                <i class="fas fa-calculator mr-1"></i> Ballni to‘g‘rilash
                                            </a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="5" class="text-center text-muted py-4">Tasdiqlangan AI resurs yo‘q.</td></tr>
                                @endforelse
                                </tbody>
                            </table>
                        </div>
